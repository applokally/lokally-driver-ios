...
class PaymentDetailsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String displayPaymentMethod =
        tripDetail?.displayPaymentMethod ?? 'cash'.tr;

    final bool shouldShowIdleFee = (tripDetail?.idleFee ?? 0) > 0;
    final bool shouldShowDelayFee = (tripDetail?.delayFee ?? 0) > 0;
    final bool shouldShowCancellationFee =
        (tripDetail?.cancellationFee ?? 0) > 0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (tripDetail?.type != 'parcel' && shouldShowIdleFee)
            PaymentItemInfoWidget(
              icon: Images.idleHourIcon,
              title: 'idle_price'.tr,
              amount: tripDetail!.idleFee ?? 0,
            ),
          if (tripDetail?.type != 'parcel' && shouldShowDelayFee)
            PaymentItemInfoWidget(
              icon: Images.waitingPrice,
              title: 'delay_price'.tr,
              amount: tripDetail!.delayFee ?? 0,
            ),
          if (tripDetail?.type != 'parcel' && shouldShowCancellationFee)
            PaymentItemInfoWidget(
              icon: Images.idleHourIcon,
