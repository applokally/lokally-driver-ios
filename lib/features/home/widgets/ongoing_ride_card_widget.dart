...
class OngoingRideCardWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<RideController>(builder: (rideController) {
      String tripDay = '0';
      String tripMonthYear = '';
      List<dynamic> extraRoute = [];
      int onGoingHr = 0, count = 1;
      if (rideController.lastRideDetails != null &&
          rideController.lastRideDetails!.isNotEmpty) {
        final String createdAt = rideController.lastRideDetails![0].createdAt!;
        final DateTime? tripCreatedAt = DateTime.tryParse(createdAt);

        if (tripCreatedAt != null) {
          tripDay = tripCreatedAt.day.toString();
          tripMonthYear = _formatTripMonthAndYear(tripCreatedAt);
        } else {
          tripDay = DateConverter.dateTimeStringToDateOnly(createdAt);
          tripMonthYear = DateConverter.dateTimeStringToMonthAndYear(createdAt);
        }

        onGoingHr =
            DateTime.now().difference(DateTime.parse(createdAt)).inMinutes;
                        ...
                        child: Row(children: [
                          Padding(
                            ...
                            child: Column(children: [
                              ...
                              Text(tripMonthYear, style: textRegular),
                            ]),
                          ),
                          const Spacer(),
                          Container(
...
String _formatTripMonthAndYear(DateTime dateTime) {
