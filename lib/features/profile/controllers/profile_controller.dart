...
class ProfileController extends GetxController implements GetxService {
  ...
  void _rebuildVehicleDocumentsMultipart() {
    documents = [];

    for (final FilePickerResult result in listOfDocuments) {
  ...
  void syncVehicleDocumentsForRequest() {
    _rebuildVehicleDocumentsMultipart();
    update();
  }
  ...
  Future<void> addNewVehicle(VehicleBody vehicleBody) async {
    ...
    Response? response = await profileServiceInterface.addNewVehicle(
      vehicleBody,
      documents,
    );
  ...
  void removeFile(int index) async {
    if (index >= 0 && index < listOfDocuments.length) {
