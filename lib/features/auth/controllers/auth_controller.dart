...
class AuthController extends GetxController implements GetxService {
  ...
  bool isParcelShare = false;
  bool isRideShare = false;

  String? _lokallyZoneId;
  String? _lokallyServiceType;
  String? _lokallyBillingMode;

  String? get lokallyZoneId => _lokallyZoneId;
  String? get lokallyServiceType => _lokallyServiceType;
  String? get lokallyBillingMode => _lokallyBillingMode;
  ...
  void setLokallyBillingRegistrationData({
    required String zoneId,
    required String serviceType,
    required String billingMode,
  }) {
    _lokallyZoneId = zoneId;
    _lokallyServiceType = serviceType;
    _lokallyBillingMode = billingMode;
    update();
  }

  void clearLokallyBillingRegistrationData() {
    _lokallyZoneId = null;
    _lokallyServiceType = null;
    _lokallyBillingMode = null;
    update();
  }

  void _applyLokallyBillingDataToSignUpBody(SignUpBody signUpBody) {
    if ((signUpBody.lokallyZoneId ?? '').isEmpty) {
      signUpBody.lokallyZoneId = _lokallyZoneId ?? '';
    }

    if ((signUpBody.lokallyServiceType ?? '').isEmpty) {
      signUpBody.lokallyServiceType = _lokallyServiceType ?? '';
    }

    if ((signUpBody.lokallyBillingMode ?? '').isEmpty) {
      signUpBody.lokallyBillingMode = _lokallyBillingMode ?? '';
    }
  }
  ...
  Future<void> login(String countryCode, String phone, String password) async {
    ...
    if (response!.statusCode == 200) {
      ...
      Get.bottomSheet(
        ManualAuthWaringBottomSheetWidget(
          phoneNumber: phone,
  ...
  Future<void> register(String code, SignUpBody signUpBody) async {
    ...
    _applyLokallyBillingDataToSignUpBody(signUpBody);
    ...
    if (response!.statusCode == 200) {
      if (configModel?.verification ?? false) {
        ...
        login(
          code,
          signUpBody.phone?.replaceAll(code, '') ?? '',
          signUpBody.password ?? '',
        );
      }
  ...
  Future<Response?> otpVerification(
    ...
    if (Get.find<SplashController>().config?.isFirebaseOtpVerification ??
      ...
      response = await authServiceInterface.verifyOtp(
        phone: phoneNumber,
        otp: otp,
      );
    }
      ...
      if (from == VerificationForm.signUp) {
        ...
        registerWithOtp(
          SignUpBody(
            ...
            lokallyZoneId: _lokallyZoneId,
            lokallyServiceType: _lokallyServiceType,
            lokallyBillingMode: _lokallyBillingMode,
          ),
