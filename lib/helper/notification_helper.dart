...
class NotificationHelper {
  static bool _isCustomerCancellationDialogOpen = false;
  static final AudioPlayer _requestAudioPlayer = AudioPlayer();
  static String? _activeRequestId;
  static final Map<String, DateTime> _handledRequestIds = <String, DateTime>{};
  static const Duration _requestDeduplicationWindow = Duration(minutes: 15);

  static Future<void> playRequestSound(String? requestId) async {
    ...
    _clearExpiredRequestIds();

    if (_handledRequestIds.containsKey(normalizedRequestId)) {
      return;
    }

    final List<dynamic>? ongoingTrip = Get.find<RideController>().ongoingTrip;
    if (ongoingTrip != null && ongoingTrip.isNotEmpty) {
      return;
    }

    _handledRequestIds[normalizedRequestId] = DateTime.now();

    if (_activeRequestId != null && _activeRequestId != normalizedRequestId) {
      await _requestAudioPlayer.stop();
    }

    _activeRequestId = normalizedRequestId;

    try {
      await _requestAudioPlayer.setReleaseMode(ReleaseMode.stop);
      await _requestAudioPlayer.play(AssetSource('notification.wav'));
    } catch (error) {
      if (_activeRequestId == normalizedRequestId) {
        _activeRequestId = null;
      }

      if (kDebugMode) {
        print('Não foi possível tocar o som da solicitação: $error');
      }
    }
  }

  static Future<void> stopRequestSound({String? requestId}) async {
    final String normalizedRequestId = requestId?.trim() ?? '';
    ...
    try {
      ...
      if (kDebugMode) {
        print('Não foi possível interromper o som da solicitação: $error');
      }
    }
  }

  static void _clearExpiredRequestIds() {
    final DateTime threshold =
        DateTime.now().subtract(_requestDeduplicationWindow);

    _handledRequestIds.removeWhere(
      (String _, DateTime handledAt) => handledAt.isBefore(threshold),
    );
  }

  static bool _isRequestAction(dynamic action) {
  ...
  static Future<void> initialize(
    ...
    if (!kIsWeb && Platform.isAndroid) {
      const AndroidNotificationChannel defaultChannel =
        ...
        'hexaride',
        'Notificações Lokally',
        description: 'Notificações gerais da Lokally Motorista.',
        importance: Importance.max,
        playSound: true,
        sound: RawResourceAndroidNotificationSound('notification'),
      );
      const AndroidNotificationChannel foregroundRequestChannel =
          AndroidNotificationChannel(
        'hexaride_foreground',
        'Solicitações em aberto',
        description: 'Alertas visuais de solicitações com o app aberto.',
        importance: Importance.max,
        playSound: false,
      );
      ...
      await androidPlugin?.createNotificationChannel(foregroundRequestChannel);
    }
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (!(Get.find<SplashController>().config!.maintenanceMode != null &&
        ...
        if (action == "new_ride_request" || action == "new_parcel_request") {
        ...
        } else if (Get.find<SplashController>().pusherConnectionStatus ==
                null ||
          ...
          if (message.data['action'] == "new_message") {
            ...
            _whenCustomerCancelTrip(message, showCancellationDialog: true);
            ...
            NotificationHelper.stopRequestSound(
              requestId: message.data['ride_request_id']?.toString(),
            );
            Get.offAll(const DashboardScreen());
        ...
        if (!(message.data['action'] == "another_driver_assigned" ||
  ...
  static Future<void> notificationToRoute(Map<String, dynamic> data,
    ...
    if (data['action'] == "new_message") {
      ...
      await showCustomerCancellationDialog(data);
    ...
    } else if (data['action'] == "another_driver_assigned") {
      Get.find<RideController>().getPendingRideRequestList(1).then((value) {
        Get.find<RideController>().tripDetail = null;
        if (value.statusCode == 200) {
          Get.find<RiderMapController>().setRideCurrentState(RideState.initial);
  ...
  static Future<void> showCustomerCancellationDialog(
      Map<String, dynamic> data) async {
    if (_isCustomerCancellationDialogOpen) {
      return;
    }

    _isCustomerCancellationDialogOpen = true;
    ...
    try {
        ...
        AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text(_customerCancellationReason(data)),
            ],
          ),
          actions: [
            TextButton(
      ...
      _isCustomerCancellationDialogOpen = false;
    }
  }

  static String _customerCancellationReason(Map<String, dynamic> data) {
    const List<String> reasonKeys = [
      'cancellation_reason',
      'cancel_reason',
      'cancellationReason',
      'cancelReason',
    ...
    for (final String key in reasonKeys) {
      final String reason = _normalizeCancellationText(data[key]);
  ...
  static String _normalizeCancellationText(dynamic value) {
...
void _whenNewRequestFound(RemoteMessage message) {
  final String requestId = message.data['ride_request_id']?.toString() ?? '';
  final bool hasCachedOngoingTrip =
      (Get.find<RideController>().ongoingTrip ?? []).isNotEmpty;

  if (!hasCachedOngoingTrip) {
    NotificationHelper.playRequestSound(requestId);
  }
  ...
  Get.find<RideController>().ongoingTripList().then((value) {
    if ((Get.find<RideController>().ongoingTrip ?? []).isEmpty) {
      ...
      NotificationHelper.stopRequestSound(requestId: requestId);
...
void _whenCustomerCancelTrip(RemoteMessage message,
    {bool showCancellationDialog = false}) {
  NotificationHelper.stopRequestSound(
    requestId: message.data['ride_request_id']?.toString(),
  );
  ...
  if (Get.find<RideController>().tripDetail?.id ==
    ...
    final Future<void> cancellationDialog = showCancellationDialog
        ? NotificationHelper.showCustomerCancellationDialog(message.data)
        : Future<void>.value();

    cancellationDialog.then((_) {
      Get.find<SafetyAlertController>().cancelDriverNeedSafetyStream();
      Get.find<RideController>().tripDetail = null;
      Get.find<RideController>().getPendingRideRequestList(1).then((value) {
        if (value.statusCode == 200) {
